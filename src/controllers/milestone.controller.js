...
export const list = async (req, res) => {
  try {
    ...
    const result = await milestoneService.listMilestones({ page, size, speedboat_id }, req.user.id);
...
export const deletedMilestoneList = async (req, res) => {
  try {
    const { page = 1, size = 25, speedboat_id } = req.query;
    const result = await milestoneService.deletedListMilestones({ page, size, speedboat_id }, req.user.id);
    return ApiResponse.ok(res, result);
  } catch (err) {
    return ApiResponse.error(res, err.message);
  }
