...
export const list = async (req, res) => {
  try {
    ...
    const result = await actionService.listNextActions({ page, limit, speedboat_id }, req.user.id);
...
export const deletedNextActionList = async (req, res) => {
  try {
    const { page = 1, limit = 25, speedboat_id } = req.query;
    const result = await actionService.deletedListNextActions({ page, limit, speedboat_id }, req.user.id);
    return ApiResponse.ok(res, result);
  } catch (err) {
    return ApiResponse.error(res, err.message);
  }
